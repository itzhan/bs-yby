      ...
      <a-spin :spinning="loading">
        <a-row :gutter="24" v-if="job">
          ...
          <a-col :xs="24" :md="16">
              ...
              <div class="detail-header">
                ...
                <div class="detail-sub-info">
                  <span v-if="job.createdAt">
                    <CalendarOutlined /> 发布于 {{ job.createdAt?.replace('T', ' ').substring(0, 10) }}
                  </span>
            ...
            >
              <template #title>
                <span><FireOutlined style="color: #ff4d4f; margin-right: 6px" />相似岗位推荐</span>
              </template>
              <div class="similar-list">
                <div
                  ...
                  :key="sj.id"
                  class="similar-item"
                  @click="router.push(`/jobs/${sj.id}`)"
                >
                  <div class="similar-title">{{ sj.title }}</div>
                  <div class="similar-company">{{ sj.companyName }}</div>
                  <div class="similar-meta">
                    <a-tag size="small" color="blue">{{ sj.city || '不限' }}</a-tag>
                    <span class="similar-salary">
                      {{ sj.salaryMin && sj.salaryMax ? `${sj.salaryMin}-${sj.salaryMax}元` : '面议' }}
                    </span>
                  </div>
                </div>
              </div>
...
import { getJobDetail, getRecommendedJobs } from '@/api/job'
...
async function fetchSimilar() {
  try {
    const res = await getRecommendedJobs(5)
    const currentId = Number(route.params.id)
    similarJobs.value = (res.data || []).filter(j => j.id !== currentId).slice(0, 4)
  } catch {
    // silent fail
  }
}
