...
const applicationColumns = [
  { title: '企业', dataIndex: 'studentName', width: 100 },
...
async function fetchDashboard() {
  ...
  try {
    ...
    const data = res.data || {}
    // 后端返回字段：totalJobs / totalApplications / applicationStatusStats / recentJobs
    stats.jobCount = data.totalJobs || 0
    stats.applicationCount = data.totalApplications || 0
    // 从 applicationStatusStats 中取 "面试中" 和 "已录用"
    const statusStats = data.applicationStatusStats || {}
    stats.interviewCount = statusStats['面试中'] || 0
    stats.hiredCount = statusStats['已录用'] || 0
    // 后端返回的是 recentJobs 而非 recentApplications，适配显示
    const jobs = (data.recentJobs || []).slice(0, 5)
    recentApplications.value = jobs.map(j => ({
      id: j.id,
      studentName: j.companyName || '-',
      jobTitle: j.title || '-',
      status: j.status,
